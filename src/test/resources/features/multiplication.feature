Feature: Users are able to send their multiplication attempts, which may be aorrect or not, When users send a correct attempt, they get a response indicating that the result is a right one. Also, they get points and potentially some badges when they are right, so they get motivation to come back and keep on playing. Badges are won for the first right attempt and when the user gets 100, 500, and 999 points respectively. If users send a wrong attempt, they don't get any points or badges.
...
Scenario: The user sends a second right attempt and gets points only
    ...
    And the user gets the FIRST_WON badge
    When the user john_snow sends 1 right attempt
    Then the user gets a response indicating the attempt is right
    And the user gets 10 points for the attempt
...
# Checks the Bronze, Silver, and Gold basges
